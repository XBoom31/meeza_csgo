    ...
    matrix3x4_t *matrix)
{
    ...
    if(is_player && vars.visuals.player.chams) {
        // 
        // Draw player Chams.
        // 
        auto ent = player_t::get_player_by_id(info.entity_index);

        if(ent && local && ent->alive()) {
            const auto enemy = ent->m_iTeamNum() != local->m_iTeamNum();
            if(!enemy && vars.visuals.player.chamsto)
                return;

            const auto clr_front = enemy ? vars.settings.colors[color_chamsev] : vars.settings.colors[color_chamsav];
            const auto clr_back = enemy ? vars.settings.colors[color_chamsen] : vars.settings.colors[color_chamsan];

            if(vars.visuals.player.chamsxqz) {
                OverrideMaterial(
                    true,
                    vars.visuals.player.chamsflat,
