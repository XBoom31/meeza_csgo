...
“meeza_csgo” este un program(cheat/hack) care ofera utilizatorului un avantaj in jocul “Counter Strike: Global Offensive”, joc dezvoltat de catre compania Valve Software, aparut in anul 2012. Povestea proiectului a inceput de la obiceiul meu de a activa pe serverele de hack vs hack, servere dedicate jucatorilor ce folosesc cheat-uri, diferenta fiind facuta de calitatea programului, deseori fiind nevoit sa platesti mai mult de $20/luna pentru a beneficia de o altfel de utilitate superioara. Asadar, pentru pentru a-mi satisface nevoia de a fi competitiv pe astfel de servere, fara a mai achizitiona un astfel de serviciu,  am decis in anul 2017 sa creez acest program. Proiectul a fost bazat pe un proiect open source numit “CSGOSimple” al utilizatorului MarkHC si cu ajutorul comunitatii forumul-ului UnknownCheats. In 2018, la apogeul proiectului, avea aproximativ 50 de utilizatori lunari la un pret de $5/luna, generand $700 in total. Cat timp proiectul a fost operational, utilizatorii s-au bucurat de nicio detectare de catre protectiile anti-cheat ale jocului. Dupa multe ore petrecute in acest joc, am renuntat la el, astfel cheat-ul fiind stopat in 2018, iar in mare parte a fost facut open source, iar restul componentelor care nu au fost publicate au fost utilizate in proiectele altor persoane(daxhack.xyz, massashook).
...
Pentru dezolvatrea cheat-ului propriu-zis a fost folosit Visual Studio utilizand limbajul C++.

Ca baza proiectului a fost folosit CSGOSimple al utilizatorului MarkHC, fiind modificat masiv si rescris tot ce nu tine de SDK. Repository-ul original a fost sters, dar exista un fork identic cu cel original.
https://github.com/spirthack/CSGOSimple

Pentru informatii a fost analizat codul source-sdk-2013, pe care este bazat jocul tinta
https://github.com/ValveSoftware/source-sdk-2013

Pentru meniu este folosita libraria Dear ImGUI, al utilizatorului Ocornut
https://github.com/ocornut/imgui

Loader-ul este creeat in Visual Studio, folosind limbajul C#, folosindu-se de framework-ul .NET.

API-urile sunt scrise in PHP.

Baza de date ruleaza mariadb.

Aplicatia companion este realizata in Android Studio, folosind limbajul Java.

III. Funtiile Programului
Functiile programului
       ...
       Legitbot mode – Selecteaza modul de operare a legitbot-ului
        ...
        Advanced – legitbot-ul foloseste o configuratie diferita pentru fiecare arma Configuratie legitbot pentru o arma:
...
Remote Config – Permite modificarea configuratiei cheat-ului din telefon. (WIP)

Remote Player list – Afiseaza o lista de jucatori cu toate informatiile utile pe telefon. (WIP)

Remote Radar – Foloseste ecranul telefonului pe post de radar al jucatorilor. (WIP)
